...
namespace Tests
{
    ...
    {
        private readonly string _subject = Assembly.GetExecutingAssembly().GetResourceString("Subject.txt");
        private readonly string _body = Assembly.GetExecutingAssembly().GetResourceString("Body.txt");

        private readonly Section _application = Manager.Current.Application;
        ...
        public void BodyParseTest()
        {
            ...
            Assert.AreEqual(values["detailsUrl"], "http://192.168.1.101:8153/go/pipelines/ProjectEuler.NET/21/CI/1");
